...
def encoder_layers(enc_inp, conv_filt, conv_act, hidden):
    ...
    enc_layers.append(Conv3D(16, (4, 4, 1), padding='valid', strides=(2,2,1), 
                        activity_regularizer=regularizers.l2(l2=L2_LOSS), activation=conv_act, 
                        name='enc_conv_0')(enc_inp))
...
def encoder_layers2D_old(enc_inp, conv_filt, conv_act, hidden):
    enc_layers = []
    enc_layers.append(Conv2D(4, (4, 4), padding='valid', strides=(2,2), 
                        activation=conv_act, name='enc_conv_0')(enc_inp))
    enc_layers.append(BatchNormalization(name='enc_batch_norm_0')(enc_layers[-1]))

    enc_layers.append(Conv2D(128, (3, 3), padding='valid', strides=(2,2), 
...
def decoder_layers2D_old(dec_inp, conv_filt, conv_act, hidden):
    dec_layers = [dec_inp]
    nconv = len(hidden)
    for i in range(nconv):
    ...
    dec_layers.append(Conv2DTranspose(128, (3, 3), padding='valid', strides=(2,2), 
                        activation=conv_act, name='dec_conv_4')(dec_layers[-1]))
    dec_layers.append(BatchNormalization(name='dec_batch_norm_3')(dec_layers[-1]))

    dec_layers.append(Conv2DTranspose(4, (3, 3), padding='valid', strides=(2,2), 
                        activation=conv_act, name='dec_conv_5')(dec_layers[-1]))
    dec_layers.append(BatchNormalization(name='dec_batch_norm_4')(dec_layers[-1]))
